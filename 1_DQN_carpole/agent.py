...
class DQN_Agent:
    def __init__(self, lr, gamma, env_class, epsilon_max, epsilon_decay, epsilon_min, hidden_layers):
        ...
        print(f'\tInput/Observation = {env_class.env.observation_space.shape[0]} | Output/action ={env_class.env.action_space.n}')
        ...
        self.Q_model = DeepQNetwork(env_class.env.observation_space.shape[0], hidden_layers, env_class.env.action_space.n, self.lr)

        self.Q_target = DeepQNetwork(env_class.env.observation_space.shape[0], hidden_layers, env_class.env.action_space.n, self.lr)
    ...
    def memory_replay(self, batch_size):
        # print('Memory replay...', end=" ")
        batch = random.sample(self.memory, batch_size)
        states, actions, rewards, next_states, dones = zip(*batch)
        self.learning(states, actions, rewards, next_states, dones)


    def learning(self, state, action, reward, state_, done):
        ...
        # print('Learning... batch size:', len(states),'| Memory size:', len(self.memory))
        # print('states', states.shape , 'actions', actions.shape, 'rewards', rewards.shape, 'states_', states_.shape, 'done', done)
            ...
            for idx in range(len(states)):
                if not dones[idx]:  # -> only do this if not done
